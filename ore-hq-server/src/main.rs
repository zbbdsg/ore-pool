...
use ore_utils::{
    ...
    get_register_ix, get_reset_ix, start_websocket_listener, get_latest_blockhash_with_retries, ORE_TOKEN_DECIMALS,
};
use rand::Rng;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ifJito = std::env::var("JITO").expect("JITO must be set.");
    ...
    if ifJito == "true" {
        ...
        start_websocket_listener(url, tip_clone).await;
    }
    ...
    let jito_rpc_client = Arc::new(jito_client);
                ...
                if let Some(solution) = solution {
                    ...
                    info!(
                        ...
                        difficulty
                            ...
                            if ifJito == "true" {
                                client = jito_rpc_client.clone();

                                let jito_tip_guard  = tip.read().await;
                                let jito_tip = *jito_tip_guard;
                                let tip_accounts = [
                            ...
                            loop {
                                ...
                                if attempts % 3 == 0 {
                                    ...
                                    match get_latest_blockhash(&client_clone).await {
                                        Ok((hash, slot)) => {
                                            println!("Hash: {}, Slot: {}", hash, slot);
                                            tx.sign(&[&signer], hash);
                                        }
                                        Err(e) => {
                                            eprintln!("Error: {:?}", e);
                                ...
                                match client.send_transaction_with_config(&tx, send_cfg).await {
                                    Ok(sig) => {
                                        ...
                                        'confirm: for _ in 0..CONFIRM_RETRIES {
                                            ...
                                            match client_clone.get_signature_statuses(&[sig]).await {
...
async fn get_latest_blockhash(client: &RpcClient) -> Result<(Hash, u64), ClientError> {
    let (hash, slot) = get_latest_blockhash_with_retries(client).await?;
    // println!("Latest blockhash: {}", hash);
    Ok((hash, slot)) // 返回 hash 和 slot
}
